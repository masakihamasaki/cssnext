...
```sh
lsw verify [--font <path>] [--dir <path>]
```

検証用の素材を ffmpeg で合成し、実際に1本焼いて出力を測る。
実素材もネットワークも要らないので CI から毎回走らせられる。

1. **フォントのグリフ被覆** — フォントの cmap を直接読み、使う文字が入っているかを判定する。
   ffmpeg はグリフが無くてもエラーにせず豆腐を焼き込むので、描画前に弾く
2. **実ビルド** — 音声あり素材と無音素材（`clipHasAudio: false`）の両方で焼く
3. **出力の検査** — 解像度・fps・pix_fmt・音声トラック・尺（フック + 本編）
4. **文字の描画確認** — フックとテロップの領域を切り出し、輝度から文字の有無を見る
5. **目視用フレーム** — `frame-hook.png` / `frame-main.png` を残す

`doctor` も同じグリフ検査を使い、設定に出てくる文字がフォントに無ければ警告する。

アップロード経路は `test/upload.test.js` がモックサーバを立てて実際に流し、
init で宣言したチャンク構成どおりに、隙間も重複もなく全バイトが届くことを確認する。
